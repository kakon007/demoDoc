...
class _VitalsWidgetState extends State<VitalsWidget> {
  ...
  @override
  void initState() {
    context.read<VitalsListSetupViewModel>().getData();
    var vitalVM = context.read<VitalsListSetupViewModel>();
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    ...
    var vitalVM = context.watch<VitalsListSetupViewModel>();
    ...
    return PrescriptionCommonWidget(
      ...
      expandedWidget: Container(
        ...
        child: Padding(
          ...
          child: Container(
            // height: vitalVM.vitalsSetupList.length * 60.0,
            child: ListView.builder(
              shrinkWrap: true,
              physics: NeverScrollableScrollPhysics(),
              itemCount: vitalVM.vitalsSetupList.length,
              itemBuilder: (context, int index) {
                return vitalVM.vitalsSetupList[index].preDiagnosisValUnit ==
                        null
                    ? Padding(
                        ...
                        child: VitalsField(
                          enabledBorderColor: "#D2D2D2",
                          labelText:
                              '${vitalVM.vitalsSetupList[index].preDiagnosisVal ?? ""}',
                          minimizeBottomPadding: true,
                          topContentPadding: isTablet ? 30 : 25,
                          hintSize: isTablet ? 17 : 12,
                          controller:
                              vitalVM.vitalsSetupList[index].vitalsController,
                        ),
                      )
                    : Padding(
                        padding: EdgeInsets.only(top: 10),
                        child: VitalsTextField(
                          levelText:
                              '${vitalVM.vitalsSetupList[index].preDiagnosisVal ?? ""}',
                          unitName:
                              '${vitalVM.vitalsSetupList[index].preDiagnosisValUnit ?? ""}',
                          topContentPadding: isTablet ? 30 : 25,
                          hintSize: isTablet ? 17 : 12,
                          unitHeight: isTablet ? 60 : 46,
                          unitWidth: isTablet ? 100 : 80,
...
//   VitalsController({this.preDiagnosisVal});
// }
